...
const features = [
  ...
  },
  {
    ...
    description: 'Track prospective tenants grouped by property with an 8-stage pipeline.',
    details: [
      'Leads grouped by property based on selected potential units',
      'If a lead has units across multiple properties, it appears under each property',
      'Unassigned Leads section at top for leads with no available units — click to add notes about what the lead is useful for',
      'Lead fields: name, source, phone, email, desired dates, price offered, preferred contact method, notes',
      'Potential units (multi-select filtered by availability during selected dates)',
      'When no units available for dates, option to save as unassigned lead',
      'Pets and parking request fields',
      'Lead strength (4 levels): Weak (red), Fair (orange), Good (yellow), Strong (green)',
      'Color-coded rows with left border bar based on strength',
