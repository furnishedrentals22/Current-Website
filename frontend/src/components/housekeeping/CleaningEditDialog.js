...
import { formatShortDate } from './housekeepingUtils';
...
export function CleaningEditDialog({
  ...
  useEffect(() => {
    if (open && record) {
      setUseCustomMaint(!record.assigned_maintenance_id && !!record.assigned_maintenance_name);
    }
  }, [open, record]);
  ...
  const modalUnit = record ? unitMap[record.unit_id] : null;
  const modalProp = record ? propMap[record.property_id] : null;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isManual ? 'Edit Manual Cleaning' : (modalUnit ? `Unit ${modalUnit.unit_number}` : 'Cleaning Details')}
            {!isManual && modalProp && (
          ...
          <DialogDescription>
            {record?.tenant_name}
            {record?.check_out_date && ` · Checkout: ${formatShortDate(record.check_out_date)}`}
            {record?.next_check_in_date && ` → Check-in: ${formatShortDate(record.next_check_in_date)}`}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-3 py-2">
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input value={form.tenant_name || ''} onChange={e => setForm(f => ({ ...f, tenant_name: e.target.value }))} placeholder="e.g. Deep clean" className="text-sm" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label className="text-xs">Date (Check-out)</Label>
                  <Input type="date" value={form.check_out_date || ''} onChange={e => setForm(f => ({ ...f, check_out_date: e.target.value }))} className="text-sm" />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs">Check-in Date</Label>
                  <Input type="date" value={form.next_check_in_date || ''} onChange={e => setForm(f => ({ ...f, next_check_in_date: e.target.value }))} className="text-sm" />
                </div>
              </div>
            </>
          )}
          ...
          <div className="space-y-1.5">
            ...
            {useCustomMaint && (
              <Input className="mt-1 text-sm" value={form.assigned_maintenance_name || ''} placeholder="Enter maintenance person name" onChange={e => setForm(f => ({ ...f, assigned_maintenance_name: e.target.value }))} data-testid="hk-custom-maint-input" />
            )}
          </div>
          ...
          {hasMaintAssigned && (
            <div className="space-y-1">
              ...
              <Input value={form.maintenance_note || ''} onChange={e => setForm(f => ({ ...f, maintenance_note: e.target.value }))} placeholder="e.g. Check kitchen faucet" data-testid="hk-maint-note-input" />
            </div>
          )}

          <div className="space-y-1">
            <Label>Notes</Label>
            <Textarea value={form.notes || ''} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={3} placeholder="Cleaning notes..." />
          </div>

          <div className="flex items-center gap-2 pt-1">
            <Checkbox id="hk-confirmed" checked={form.confirmed || false} onCheckedChange={v => setForm(f => ({ ...f, confirmed: v }))} data-testid="cleaning-confirmed-checkbox" />
        ...
        <DialogFooter>
          ...
          <Button onClick={onSave} disabled={saving} data-testid="hk-edit-save-btn">
            {saving ? 'Saving...' : 'Save'}
          </Button>
        </DialogFooter>
      </DialogContent>
